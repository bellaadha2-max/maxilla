    ...
    <div class="lg:col-span-1">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 flex flex-col items-center text-center">
            <img class="w-32 h-32 rounded-full border-4 border-slate-50 shadow-md bg-blue-100 object-cover mb-4"
                 src="https://ui-avatars.com/api/?name={{ urlencode($dokter->nama) }}&background=dbeafe&color=1d4ed8&size=128" alt="Avatar">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div><p class="text-slate-400 text-xs uppercase font-black">Email</p><p class="font-bold text-slate-800">{{ $dokter->email }}</p></div>
                <div><p class="text-slate-400 text-xs uppercase font-black">WhatsApp</p><p class="font-bold text-slate-800">{{ $dokter->no_wa }}</p></div>
                <div><p class="text-slate-400 text-xs uppercase font-black">Penempatan</p><p class="font-bold text-slate-800">Multi Cabang</p></div>
                <div><p class="text-slate-400 text-xs uppercase font-black">Status</p><p class="font-bold {{ $dokter->is_active ? 'text-emerald-600' : 'text-red-600' }}">{{ $dokter->is_active ? 'Aktif' : 'Non-aktif' }}</p></div>
            </div>
        </div>
