...
@section('title', 'Edit Akun ' . $roleLabel)

@section('content')
<div class="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-5 relative z-10">
    <div>
        <h1 class="font-heading text-3xl font-bold text-slate-800 tracking-tight">Edit {{ $roleLabel }}</h1>
        <p class="text-slate-500 mt-1 text-sm">Perbarui informasi akun {{ strtolower($roleLabel) }}: <span class="font-bold text-slate-700">{{ $staff->nama }}</span></p>
    </div>
...
<div class="max-w-4xl">
    <form action="{{ route('superadmin.' . $roleSlug . '.update', $staff->id_user) }}" method="POST" class="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $staff->nama) }}" required class="w-full px-4 py-3 rounded-2xl border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-medium transition-all">
                ...
                <div>
                    ...
                    <input type="text" name="no_wa" value="{{ old('no_wa', $staff->no_wa) }}" required class="w-full px-4 py-3 rounded-2xl border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-medium transition-all">
                ...
                <div>
                    ...
                    <select name="cabang" required class="w-full px-4 py-3 rounded-2xl border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-bold transition-all appearance-none bg-white">
                        <option value="slawi" {{ old('cabang', $staff->cabang) == 'slawi' ? 'selected' : '' }}>Klinik Slawi</option>
                        <option value="tegal" {{ old('cabang', $staff->cabang) == 'tegal' ? 'selected' : '' }}>Klinik Tegal</option>
                        <option value="brebes" {{ old('cabang', $staff->cabang) == 'brebes' ? 'selected' : '' }}>Klinik Brebes</option>
                    </select>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $staff->email) }}" required class="w-full px-4 py-3 rounded-2xl border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-medium transition-all">
                    @error('email') <p class="text-red-500 text-[10px] mt-1 font-bold">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">Password Baru (Opsional)</label>
                    <input type="password" name="password" class="w-full px-4 py-3 rounded-2xl border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-medium transition-all" placeholder="Kosongkan jika tidak diubah">
                ...
                <div>
                    ...
                    <input type="password" name="password_confirmation" class="w-full px-4 py-3 rounded-2xl border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-medium transition-all" placeholder="Ulangi password baru">
                </div>
            </div>
        </div>
