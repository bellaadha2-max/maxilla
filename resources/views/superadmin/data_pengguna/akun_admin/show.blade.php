...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-6xl">
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 flex flex-col items-center text-center">
            <img class="w-32 h-32 rounded-full border-4 border-slate-50 shadow-md bg-indigo-100 object-cover mb-4" 
                 src="https://ui-avatars.com/api/?name={{ urlencode($admin->nama) }}&background=e0e7ff&color=4338ca&size=128" alt="Avatar">
            <h2 class="text-xl font-black text-slate-800">{{ $admin->nama }}</h2>
            <p class="text-indigo-600 font-bold text-sm mt-1 uppercase tracking-wider">Admin {{ ucfirst($admin->cabang) }}</p>
            
            <div class="mt-6 w-full pt-6 border-t border-slate-100">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full {{ $admin->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700' }} text-[11px] font-bold">
                    <span class="w-1.5 h-1.5 rounded-full {{ $admin->is_active ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
    ...
    <div class="lg:col-span-2 space-y-6">
                ...
                <div class="overflow-hidden border border-slate-100 rounded-xl">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <tr>
                                <td class="px-4 py-4 font-medium text-slate-700">{{ $admin->last_login_at ?? 'Belum pernah login' }}</td>
        ...
        <div class="flex items-center justify-end gap-3">
            <form action="{{ route('superadmin.admin.destroy', $admin->id_user) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun ini?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-6 py-2.5 border border-red-200 text-red-600 rounded-xl text-sm font-bold hover:bg-red-50 transition-all flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    Hapus Akun
                </button>
            </form>
            <a href="{{ route('superadmin.admin.edit', $admin->id_user) }}" class="px-8 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-md flex items-center gap-2">
